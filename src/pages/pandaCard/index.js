...
export default function() {
  ...
  async function getQuery(){
      ...
      render: rs => (
		...
		},
		{
          ...
          <span style={{color: "#D9D9D9", fontSize: "12px", cursor: 'pointer'}} onClick={async () => {
            let res = await $.post("/bankcard/applicant/unprocessed", {user_uuid: record.user_uuid});
            tableList.reload();
            $.msg("操作成功~");
            return res;
          }}>标记处理</span>: 
          <a
						onClick={async () => {
							let res = await $.post("/bankcard/applicant/processed", {user_uuid: record.user_uuid});
							tableList.reload();
							$.msg("操作成功~");
							return res;
						}}
	...
	return (
      ...
      <TablePagination api="/bankcard/applicants" columns={columns} ref={(rs) => tableList = rs} />
